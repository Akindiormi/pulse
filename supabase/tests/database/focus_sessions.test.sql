...
select has_function('public', 'validate_focus_session_transition', ARRAY[]::text[], 'focus lifecycle validation trigger function exists');
...
select is((
  ...
  where child.relname = 'focus_sessions' and parent.relname = 'tasks' and c.contype = 'f'
  limit 1
...
insert into focus_test_users(name, id) select 'owner', id from auth.users where email = 'focus-test-owner@example.com';
insert into focus_test_users(name, id) select 'other', id from auth.users where email = 'focus-test-other@example.com';
...
insert into public.focus_sessions (user_id, task_id, status, planned_duration_seconds, started_at, active_duration_seconds)
select (select id from focus_test_users where name = 'owner'), id, 'running', 1800, now() - interval '1500 seconds', 0
from public.tasks where title = 'Focus foundation test task';

update focus_test_rows set session_id = (
  select id from public.focus_sessions where task_id = focus_test_rows.task_id order by created_at desc limit 1
);

select is((select count(*) from public.focus_sessions where user_id = (select id from focus_test_users where name = 'owner')), 1::bigint, 'owner can create a focus session');
...
update public.focus_sessions set status = 'paused', active_duration_seconds = 1500 where id = (select session_id from focus_test_rows);
select is((select status from public.focus_sessions where id = (select session_id from focus_test_rows)), 'paused', 'running session can transition to paused');

update public.focus_sessions set status = 'running' where id = (select session_id from focus_test_rows);
select is((select status from public.focus_sessions where id = (select session_id from focus_test_rows)), 'running', 'paused session can resume to running');

select throws_ok(
  $$update public.focus_sessions set active_duration_seconds = -1 where id = (select session_id from focus_test_rows)$$,
  'Active duration cannot be negative', 'negative active duration is rejected'
);
...
select throws_ok(
  ...
  'ended_at cannot be before started_at', 'ended_at before started_at is rejected'
);

update public.focus_sessions set status = 'completed', ended_at = now(), active_duration_seconds = 1500 where id = (select session_id from focus_test_rows);
select is((select status from public.focus_sessions where id = (select session_id from focus_test_rows)), 'completed', 'running session can complete with persisted active duration');

select throws_ok(
  $$update public.focus_sessions set status = 'running' where id = (select session_id from focus_test_rows)$$,
  'Terminal focus sessions cannot change lifecycle state', 'completed sessions cannot resume'
);

select set_config('request.jwt.claim.sub', (select id::text from focus_test_users where name = 'other'), true);
select is((select count(*) from public.focus_sessions where id = (select session_id from focus_test_rows)), 0::bigint, 'another authenticated user cannot read the owner session');

update public.focus_sessions set active_duration_seconds = 1499 where id = (select session_id from focus_test_rows);
select is((select active_duration_seconds from public.focus_sessions where id = (select session_id from focus_test_rows)), null::integer, 'another authenticated user cannot mutate the owner session');

select set_config('request.jwt.claim.sub', (select id::text from focus_test_users where name = 'owner'), true);
delete from public.tasks where id = (select task_id from focus_test_rows);

select is((select task_id from public.focus_sessions where id = (select session_id from focus_test_rows)), null::uuid, 'deleting a task clears task_id but preserves the Focus session');
select is((select status from public.focus_sessions where id = (select session_id from focus_test_rows)), 'completed', 'historical Focus session remains after task deletion');
