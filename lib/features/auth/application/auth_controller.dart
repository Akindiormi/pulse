...
class AuthController extends Notifier<AuthControllerState> {
  ...
  Future<AuthState?> signIn(String email, String password) async {
    if (state.loading) return null; state = state.copyWith(loading: true, error: null);
    ...
    catch (e) { final error = ErrorMessageMapper.from(e, kind: AppErrorKind.auth); state = state.copyWith(loading: false, error: error); await ref.read(analyticsServiceProvider).logAuthFailed('email_sign_in'); return null; }
  }
  Future<AuthState?> signUp(String email, String password) async {
    ...
    catch (e) { final error = ErrorMessageMapper.from(e, kind: AppErrorKind.auth); state = state.copyWith(loading: false, error: error); await ref.read(analyticsServiceProvider).logAuthFailed('email_sign_up'); return null; }
  }
  Future<AppError?> resetPassword(String email) async {
    ...
    catch (e) { final error = ErrorMessageMapper.from(e, kind: AppErrorKind.auth); state = state.copyWith(loading: false, error: error); await ref.read(analyticsServiceProvider).logAuthFailed('password_reset'); return error; }
  }
  void clearError() => state = state.copyWith(error: null);
}
class AuthControllerState {
  const AuthControllerState({this.loading = false, this.error}); final bool loading; final AppError? error;
