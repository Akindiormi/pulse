...
class _AuthScreenState extends ConsumerState<AuthScreen> {
  ...
  final emailFocus = FocusNode(), passwordFocus = FocusNode(); bool obscure = true;
  ...
  String? validateEmail() { final v = email.text.trim(); if (v.isEmpty) return 'enter your email address.'; if (!RegExp(r'^[^@\s]+@[^@\s]+\.[^@\s]+$').hasMatch(v)) return 'enter a valid email address.'; return null; }
  ...
  Future<void> submit() async {
    FocusScope.of(context).unfocus(); final ee = validateEmail(); final pe = mode == _AuthMode.reset ? null : validatePassword(); final mismatch = mode == _AuthMode.signUp && password.text != confirm.text;
    if (ee != null || pe != null || mismatch) { setState(() {}); return; }
    final controller = ref.read(authControllerProvider.notifier); final analytics = ref.read(analyticsServiceProvider);
    ...
    final result = signUp ? await controller.signUp(email.text, password.text) : await controller.signIn(email.text, password.text); if (!mounted || result == null) return;
    if (result.status == AuthStatus.authenticatedUnverified) { try { await ref.read(authServiceProvider).sendEmailVerification(); await analytics.logEmailVerificationSent(); } catch (_) {} if (mounted) context.go('/verify-email'); return; }
    if (result.status == AuthStatus.authenticated) { if (signUp) await analytics.logSignUp(); else await analytics.logLogin(); if (mounted) context.go('/splash'); }
  }
  void _show(String message) => ScaffoldMessenger.of(context)..clearSnackBars()..showSnackBar(SnackBar(content: Text(message)));
  @override Widget build(BuildContext context) {
    final auth = ref.watch(authControllerProvider); ref.listen<AuthControllerState>(authControllerProvider, (_, next) { final m = next.error?.message; if (m != null && m.isNotEmpty && mounted) _show(m); });
    ...
    return Scaffold(body: SafeArea(child: Form(child: ListView(padding: const EdgeInsets.all(PulseSpace.xl), children: [
      IconButton(alignment: Alignment.centerLeft, onPressed: () => setState(() => mode = _AuthMode.entry), icon: const Icon(Icons.arrow_back), tooltip: 'back'), const SizedBox(height: PulseSpace.xl),
      Text(title, style: AppTypography.display), const SizedBox(height: PulseSpace.sm), Text(mode == _AuthMode.reset ? 'we’ll send a reset link to your email.' : 'your progress starts here.', style: AppTypography.body.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)), const SizedBox(height: PulseSpace.xxl),
      TextField(controller: email, focusNode: emailFocus, keyboardType: TextInputType.emailAddress, textInputAction: mode == _AuthMode.reset ? TextInputAction.done : TextInputAction.next, autofillHints: const [AutofillHints.email], decoration: InputDecoration(labelText: 'email', errorText: validateEmail()), onChanged: (_) => setState(() {}), onSubmitted: (_) => mode == _AuthMode.reset ? submit() : passwordFocus.requestFocus()),
