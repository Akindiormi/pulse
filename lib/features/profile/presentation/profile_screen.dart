...
class _Loading extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(
        ...
        children: const [
          PulseCardLoading(height: 220),
          SizedBox(height: PulseSpace.lg),
          PulseCardLoading(height: 150),
          SizedBox(height: PulseSpace.md),
          PulseCardLoading(height: 150),
        ],
      );
}
...
class _Profile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        padding: const EdgeInsets.fromLTRB(PulseSpace.lg, PulseSpace.md, PulseSpace.lg, PulseSpace.xxxl),
        children: [
          PulseMotionBoundaryV2(
            intent: PulseMotionIntent.profileEntrance,
            state: PulseProfileMotionState.idle,
            child: _IdentityCard(
              data: data,
              onEdit: () => _showEditProfile(context, ref, data.user),
            ),
          ),
          ...
          PulseMotionBoundaryV2(
            ...
            child: _ProgressionCard(user: data.user),
          ),
          const SizedBox(height: PulseSpace.lg),
          ...
          PulseMotionBoundaryV2(
            ...
            state: PulseAchievementMotionState.unlocked,
            child: _AchievementCard(data: data),
          ),
          ...
          return SafeArea(
            child: Padding(
              padding: EdgeInsets.fromLTRB(
                ...
                PulseSpace.md,
                PulseSpace.xl,
                MediaQuery.viewInsetsOf(context).bottom + PulseSpace.xl,
              ),
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'how should Pulse call you?',
                      ),
                    ),
                    if (editState == ProfileEditState.error) ...[
                      const SizedBox(height: PulseSpace.sm),
                      Semantics(
                        liveRegion: true,
                        child: Text(
                          controller.editError ?? 'we couldn’t save your profile.',
                          style: AppTypography.metadata.copyWith(color: PulseColors.error),
                        ),
                      ),
                    ],
                    ...
                    PulseMotionBoundaryV2(
                      ...
                      state: saving
                          ? PulseProfileMotionState.saving
                          : editState == ProfileEditState.saved
                              ? PulseProfileMotionState.saved
                              : PulseProfileMotionState.idle,
                      child: PulseButton(
  ...
  Future<void> _confirmSignOut(BuildContext context, WidgetRef ref) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('you’ll need to sign in again to continue your Pulse journey.'),
        actions: [
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('we couldn’t sign you out. please try again.')),
          );
        }
      }
    }
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'this is permanent. Pulse will ask the current authentication service to delete your account. profile data will be removed through the trusted deletion workflow.',
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('cancel')),
    ...
    if (confirmed == true) {
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('we couldn’t delete your account. please try again or re-authenticate.')),
          );
        }
      }
    }
...
class _IdentityCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      label: 'profile identity for $display',
      child: PulseCard(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
              ...
              children: [
                PulseMotionAttachment(
                  ...
                  child: Semantics(
                    container: true,
                    label: hasPhoto ? 'profile photo' : 'profile avatar for $display',
                    image: true,
                    child: CircleAvatar(
                      radius: 38,
                      ...
                      child: hasPhoto
                          ...
                          : Text(
                              initials,
                              style: AppTypography.title.copyWith(color: PulseColors.accent),
                            ),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (user.username?.trim().isNotEmpty ?? false) ...[
                      ...
                      const SizedBox(height: PulseSpace.sm),
                      Text(
                        user.createdAt == null ? 'building your journey' : 'your Pulse journey',
                        style: AppTypography.metadata,
                      ),
                    ],
                  ),
            ...
            const SizedBox(height: PulseSpace.xs),
            Text(
              'your progress, streaks and milestones live here.',
              style: AppTypography.metadata,
            ),
          ],
        ),
    ...
    return PulseCard(
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: Text('level ${user.level}', style: AppTypography.title)),
              Text('${user.xp} XP', style: AppTypography.numberSmall),
            ],
          ),
          const SizedBox(height: PulseSpace.md),
          Semantics(
            label: '${user.xp} XP progression toward level ${user.level + 1}',
            value: '${(progress * 100).round()} percent',
            child: TweenAnimationBuilder<double>(
              tween: Tween(begin: 0, end: progress),
              duration: PulseMotionPolicy.transitionDuration(context),
              ...
              Expanded(
                child: _Stat(
                  ...
                  state: user.currentStreak > 0
                      ? PulseStreakMotionState.active
                      : PulseStreakMotionState.inactive,
                ),
              ),
        ...
        child: Row(
          children: [
            Container(
              width: 44,
              height: 44,
              decoration: BoxDecoration(shape: BoxShape.circle, color: PulseColors.accentTint),
              child: const Icon(Icons.explore_outlined, color: PulseColors.accent),
            ),
            const SizedBox(width: PulseSpace.lg),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('your journey', style: AppTypography.title),
                  const SizedBox(height: PulseSpace.xs),
                  Text('${user.completedCategories.length} categories explored', style: AppTypography.body),
                  const SizedBox(height: 2),
                  Text(
                    user.completedCategories.isEmpty
                        ? 'your first category is waiting.'
                        : 'keep exploring different sides of Pulse.',
                    style: AppTypography.metadata,
                  ),
                ],
              ),
          ...
          (record) => Padding(
            ...
            child: Semantics(
              ...
              child: Container(
                ...
                child: const Icon(Icons.workspace_premium_rounded, color: PulseColors.accent),
              ),
            ),
          ),
        )
      ...
      child: PulseCard(
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: Text('achievements', style: AppTypography.title)),
                Text('${data.unlockedAchievementCount} unlocked', style: AppTypography.metadata),
              ],
            ),
            const SizedBox(height: PulseSpace.lg),
            if (badges.isEmpty)
...
class _Stat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.numberSmall),
            const SizedBox(height: 2),
