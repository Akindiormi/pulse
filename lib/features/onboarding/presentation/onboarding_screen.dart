...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  Future<void> finish({required bool skipped}) async {
    if (skipped) await ref.read(onboardingControllerProvider.notifier).skip();
    else await ref.read(onboardingControllerProvider.notifier).complete();
    if (mounted) context.go('/auth');
  }

  @override
  Widget build(BuildContext context) {
    final reducedMotion = PulseMotionPolicy.userReducedMotion;
    ...
    return Scaffold(body: SafeArea(child: Padding(
      ...
      child: Column(children: [
        Row(children: [Text('PULSE', style: AppTypography.title.copyWith(fontWeight: FontWeight.w900)), const Spacer(), if (page < pages.length - 1) TextButton(onPressed: () => finish(skipped: true), child: const Text('skip'))]),
        Expanded(child: AnimatedSwitcher(duration: reducedMotion ? Duration.zero : const Duration(milliseconds: 260), child: KeyedSubtree(key: ValueKey(page), child: Center(child: SingleChildScrollView(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: PulseSpace.xxl), Text(item.$1, textAlign: TextAlign.center, style: AppTypography.display), const SizedBox(height: PulseSpace.md), Text(item.$2, textAlign: TextAlign.center, style: AppTypography.body.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        ]))))),
        ...
        const SizedBox(height: PulseSpace.lg), SizedBox(width: double.infinity, child: FilledButton(onPressed: () => page == pages.length - 1 ? finish(skipped: false) : setState(() => page++), child: Text(page == pages.length - 1 ? 'get started' : 'continue'))),
      ]),
    )));
  }
}
