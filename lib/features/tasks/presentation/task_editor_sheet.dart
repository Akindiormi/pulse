...
class _TaskEditorSheetState extends State<TaskEditorSheet> {
  ...
  Future<void> _pickUntil() async {
    ...
    if (picked != null) setState(() => _until = DateTime(picked.year, picked.month, picked.day));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Padding(
        padding: EdgeInsets.only(left: PulseSpace.lg, right: PulseSpace.lg, top: PulseSpace.lg, bottom: MediaQuery.viewInsetsOf(context).bottom + PulseSpace.lg),
        child: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: [
              Row(children: [Expanded(child: Text(_editing ? 'Edit task' : 'New task', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800))), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded))]),
              const SizedBox(height: PulseSpace.md),
              TextField(controller: _title, autofocus: !_editing, textCapitalization: TextCapitalization.sentences, decoration: const InputDecoration(labelText: 'Task', hintText: 'What needs to get done?')),
              const SizedBox(height: PulseSpace.md),
              DropdownButtonFormField<String?>(
                value: _milestoneId,
                decoration: const InputDecoration(labelText: 'Milestone'),
                items: [const DropdownMenuItem<String?>(value: null, child: Text('No milestone')), ...widget.milestones.map((milestone) => DropdownMenuItem<String?>(value: milestone.id, child: Text(milestone.name)))],
                onChanged: (value) => setState(() => _milestoneId = value),
              ),
              const SizedBox(height: PulseSpace.md),
              Row(children: [Expanded(child: OutlinedButton.icon(onPressed: _pickDate, icon: const Icon(Icons.calendar_today_rounded), label: Text(_dateLabel(_date)))), const SizedBox(width: PulseSpace.sm), Expanded(child: TextField(controller: _time, keyboardType: TextInputType.datetime, decoration: const InputDecoration(labelText: 'Time', hintText: '09:00')))]),
              if (recurringEnabled) ...[
                ...
                if (_recurrence != null) ...[
                  const SizedBox(height: PulseSpace.md),
                  TextField(controller: _interval, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: _recurrence == TaskRecurrenceType.weekly ? 'Every how many weeks?' : 'Every how many days?')),
                  const SizedBox(height: PulseSpace.md),
                  Text('Starts ${_dateLabel(_date)} · ${widget.timezone}', style: Theme.of(context).textTheme.bodySmall),
                  const SizedBox(height: PulseSpace.md),
                  CheckboxListTile(contentPadding: EdgeInsets.zero, value: _hasEndDate, title: const Text('End on a date'), onChanged: (value) => setState(() => _hasEndDate = value ?? false)),
                  if (_hasEndDate) OutlinedButton.icon(onPressed: _pickUntil, icon: const Icon(Icons.event_rounded), label: Text(_until == null ? 'Choose end date' : 'Ends ${_dateLabel(_until!)}')),
                  CheckboxListTile(contentPadding: EdgeInsets.zero, value: _hasCount, title: const Text('End after a number of occurrences'), onChanged: (value) => setState(() => _hasCount = value ?? false)),
                  if (_hasCount) TextField(controller: _count, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Number of occurrences')),
              ...
              const SizedBox(height: PulseSpace.xl),
              SizedBox(width: double.infinity, child: FilledButton(onPressed: _save, child: Text(_editing ? 'Save changes' : 'Create task'))),
            ],
          ),
        ),
      ),
  ...
  String _weekday(DateTime value) => const ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][value.weekday - 1];
}
