  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: state.when(
        ...
        data: (data) => _Collection(data: data),
      ),
    );
  }
}
...
class _Collection extends StatelessWidget {
  const _Collection({required this.data});
  final AchievementsViewData data;

  @override
  Widget build(BuildContext context) {
    final xp = XPService();
    final progress = xp.progress(data.user.xp);
    final nextXp = xp.nextLevelXP(data.user.xp);
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.fromLTRB(PulseSpace.lg, PulseSpace.md, PulseSpace.lg, PulseSpace.xxxl), children: [
        ...
        if (data.items.isEmpty)
          ...
          _SectionTitle(title: 'unlocked', count: data.unlocked.length),
          const SizedBox(height: PulseSpace.md),
          if (data.unlocked.isEmpty)
            const PulseEmptyState(title: 'nothing unlocked yet.', message: 'your first challenge is the first badge waiting for you.')
          else
            _Grid(items: data.unlocked, newlyUnlockedIds: data.newlyUnlockedIds),
          const SizedBox(height: PulseSpace.xxl),
          _SectionTitle(title: 'locked', count: data.locked.length),
          const SizedBox(height: PulseSpace.md),
          _Grid(items: data.locked, newlyUnlockedIds: data.newlyUnlockedIds),
        ],
      ]),
    );
  }
  ...
  Widget build(BuildContext context) => PulseCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Text('${user.xp} XP', style: AppTypography.metadata.copyWith(fontWeight: FontWeight.w700)),
          ]),
          const SizedBox(height: PulseSpace.md),
          ...
          Text('$nextLevelXP XP to next level', style: AppTypography.metadata),
          ...
          Row(children: [
            ...
            Expanded(child: _Metric(label: 'longest streak', value: '${user.longestStreak} days', state: user.longestStreak > 0 ? PulseStreakMotionState.milestone : PulseStreakMotionState.inactive)),
            const SizedBox(width: PulseSpace.md),
...
class _Grid extends StatelessWidget {
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
    ...
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: columns, crossAxisSpacing: PulseSpace.md, mainAxisSpacing: PulseSpace.md, childAspectRatio: .88),
      itemBuilder: (_, index) => _AchievementTile(item: items[index], newlyUnlocked: newlyUnlockedIds.contains(items[index].definition.id)),
    );
  });
}
...
class _AchievementTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      button: true,
      label: '${item.definition.name}, ${item.unlocked ? 'unlocked' : 'locked'}: ${item.definition.description}',
      child: InkWell(
        ...
        child: PulseMotionSlot(child: AnimatedContainer(
          duration: PulseMotionPolicy.duration(context, const Duration(milliseconds: 220)),
          padding: const EdgeInsets.all(PulseSpace.lg),
          decoration: BoxDecoration(color: item.unlocked ? PulseColors.accentTint : Theme.of(context).colorScheme.surfaceContainerHighest, borderRadius: BorderRadius.circular(PulseRadius.large)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _BadgeIcon(unlocked: item.unlocked, newlyUnlocked: newlyUnlocked, motion: motion),
            const Spacer(),
            Text(item.definition.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: AppTypography.body.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: PulseSpace.xs),
            Text(item.unlocked ? 'unlocked' : 'locked', style: AppTypography.metadata),
...
class _BadgeIcon extends StatelessWidget {
  const _BadgeIcon({required this.unlocked, required this.newlyUnlocked, required this.motion});
  final bool unlocked, newlyUnlocked;
...
class _AchievementDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(PulseSpace.xl, PulseSpace.md, PulseSpace.xl, PulseSpace.xxxl), child: Semantics(
      label: '${item.definition.name} details, ${item.unlocked ? 'unlocked' : 'locked'}',
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Text(item.unlocked ? (newlyUnlocked ? 'newly unlocked' : 'achievement unlocked') : 'locked', style: AppTypography.metadata.copyWith(color: PulseColors.accent, fontWeight: FontWeight.w700)),
        const SizedBox(height: PulseSpace.md),
        Text(item.definition.name, style: AppTypography.headline),
        const SizedBox(height: PulseSpace.sm),
        Text(item.definition.description, style: AppTypography.body),
        const SizedBox(height: PulseSpace.lg),
        ...
        if (item.unlocked && date != null) Text('unlocked on ${_date(date)}', style: AppTypography.metadata),
        const SizedBox(height: PulseSpace.md),
        Text('+${item.definition.xpReward} XP', style: AppTypography.body.copyWith(fontWeight: FontWeight.w700)),
      ]),
    )));
  }
