...
CustomTransitionPage<void> _motionPage({required BuildContext context, required Widget child, required LocalKey key, Offset begin = const Offset(0, .04), bool scale = false}) {
  ...
  return CustomTransitionPage<void>(key: key, transitionDuration: duration, reverseTransitionDuration: duration, child: child, transitionsBuilder: (context, animation, secondaryAnimation, child) {
    final curved = CurvedAnimation(parent: animation, curve: PulseMotionPolicy.curve(context), reverseCurve: PulseMotionPolicy.curve(context, normal: Curves.easeIn));
    ...
    final transformed = scale ? ScaleTransition(scale: Tween<double>(begin: .985, end: 1).animate(curved), child: child) : child;
...
final appRouter = GoRouter(initialLocation: '/splash', routes: [
  ...
  ShellRoute(builder: (_, state, child) => PulseShell(currentPath: state.uri.path, child: child), routes: [
    ...
    GoRoute(path: '/progress', pageBuilder: (context, state) => _motionPage(context: context, key: state.pageKey, child: const ProgressScreen(), begin: const Offset(.03, 0), scale: true)),
