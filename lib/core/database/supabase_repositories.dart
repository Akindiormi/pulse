...
class SupabaseUserRepository implements UserRepository {
  ...
  @override
  Future<void> createOrUpdateUser({required String uid, String? displayName, String? photoUrl}) async => await supabase.from('profiles').upsert({'id': uid, if (displayName != null) 'display_name': displayName, if (photoUrl != null) 'photo_url': photoUrl});

  @override
  Future<void> updateProfileFields({required String uid, required Map<String, dynamic> fields}) async => await supabase.from('profiles').update(_toSnakeCase(UserProfileUpdate(fields).toFirestore())).eq('id', uid);
...
class SupabaseChallengeRepository implements ChallengeRepository {
  ...
  Future<Map<String, dynamic>?> getDailyAssignment({required String uid, required String date}) async {
    ...
    return {'challengeId': row['challenge_id'], 'date': row['date'], 'assignedAt': row['assigned_at'], 'completedAt': row['completed_at'], 'completed': row['completed']};
  }
...
class SupabaseActivityRepository implements ActivityRepository {
  ...
  Future<List<ActivityModel>> getActivities(String uid) async {
    ...
    return rows.map((row) => ActivityModel.fromMap(row['id'] as String, {'userId': row['user_id'], 'challengeId': row['challenge_id'], 'date': row['date'], 'xpAwarded': row['xp_awarded'], 'completedAt': row['completed_at'], 'category': row['category']})).toList(growable: false);
  }
  @override
  Future<Set<String>> getCompletedCategories(String uid) async { final rows = await supabase.from('activities').select('category').eq('user_id', uid); return rows.map((row) => row['category']).whereType<String>().toSet(); }
}

class SupabaseTaskRepository implements TaskRepository {
  SupabaseTaskRepository(this.supabase);
  final SupabaseClient supabase;

  @override
  Future<List<Task>> getTasks({required String uid}) async {
    final rows = await supabase.from('tasks').select().eq('user_id', uid).order('due_date', nullsFirst: false).order('due_time', nullsFirst: false).order('created_at');
    return rows.map((row) => Task.fromMap(row['id'] as String, row)).toList(growable: false);
  }

  @override
  Future<Task> createTask({required String uid, required String title, DateTime? dueDate, String? dueTime, int priority = 0}) async {
    final row = await supabase.from('tasks').insert({'user_id': uid, 'title': title.trim(), if (dueDate != null) 'due_date': dueDate.toIso8601String().split('T').first, if (dueTime != null) 'due_time': dueTime, 'priority': priority}).select().single();
    return Task.fromMap(row['id'] as String, row);
  }
...
class SupabaseAchievementRepository implements AchievementRepository {
  ...
  final SupabaseClient supabase;
  @override
  Future<Set<String>> getUnlockedIds(String uid) async { final rows = await supabase.from('achievements').select('achievement_id').eq('user_id', uid); return rows.map((row) => row['achievement_id']).whereType<String>().toSet(); }
  @override
  Future<List<AchievementRecord>> getUnlockedRecords(String uid) async { final rows = await supabase.from('achievements').select().eq('user_id', uid); return rows.map((row) => AchievementRecord.fromMap(row['achievement_id'] as String, {'achievementId': row['achievement_id'], 'unlockedAt': row['unlocked_at']})).toList(growable: false); }
}
