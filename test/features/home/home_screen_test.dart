...
void main() {
  ...
  testWidgets('completed tasks are visibly completed', (tester) async { await tester.pumpWidget(_app(AsyncData(_data(tasks: [_task('task-2', 'done task', status: TaskStatus.completed, completedAt: DateTime.now())])))); await tester.pump(); final text = tester.widget<Text>(find.text('done task')); expect(text.style?.decoration, TextDecoration.lineThrough); });
