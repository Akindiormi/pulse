...
Task _task(String id, String title, {TaskStatus status = TaskStatus.todo, DateTime? dueDate, int priority = 0, DateTime? completedAt}) => Task(id: id, userId: 'user-1', title: title, status: status, dueDate: dueDate ?? DateTime.now(), priority: priority, completedAt: completedAt);
...
HomeViewData _data({List<Task>? tasks, List<FocusSession>? focusSessions}) => HomeViewData(user: _user(), tasks: tasks ?? <Task>[_task('task-1', 'finish pharmacology assignment', priority: 1)], projects: const [], focusSessions: focusSessions ?? const []);
