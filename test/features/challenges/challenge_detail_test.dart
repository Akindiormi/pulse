...
void main() {
  ...
  testWidgets('active state exposes completion action', (tester) async { await tester.pumpWidget(_app(AsyncData(_data(phase: ChallengeDetailPhase.active)))); await tester.pump(); expect(find.text('complete challenge'), findsOneWidget); expect(find.textContaining('tap complete'), findsOneWidget); });
  testWidgets('completing state disables the primary action and shows pending feedback', (tester) async { await tester.pumpWidget(_app(AsyncData(_data(phase: ChallengeDetailPhase.completing)))); await tester.pump(); expect(find.text('checking your completion…'), findsOneWidget); });
  testWidgets('successful authoritative result displays XP, streak, achievement and level-up', (tester) async { final result = CompletionResult(completed: true, alreadyCompleted: false, xpAwarded: 75, previousXP: 90, currentXP: 165, previousStreak: 3, currentStreak: 4, longestStreak: 4, previousLevel: 1, newLevel: 2, leveledUp: true, newAchievements: const ['FIRST_STEP'], events: const [], challengeId: 'challenge-1'); await tester.pumpWidget(_app(AsyncData(_data(phase: ChallengeDetailPhase.completed, completion: result)))); await tester.pump(); expect(find.text('+75 XP\nstreak: 4 days\nachievement unlocked: First Step\nlevel 1 → level 2'), findsOneWidget); expect(find.text('level up.'), findsOneWidget); expect(find.text('you reached level 2'), findsOneWidget); });
  testWidgets('already completed state never presents a new reward', (tester) async { await tester.pumpWidget(_app(AsyncData(_data(phase: ChallengeDetailPhase.alreadyCompleted)))); await tester.pump(); expect(find.text('already done.'), findsOneWidget); expect(find.textContaining('no new reward was added'), findsOneWidget); expect(find.textContaining('+25 XP'), findsNothing); });
  testWidgets('generic error renders safe retry copy', (tester) async { const error = TrustedBackendException(TrustedBackendErrorCode.internal, 'sensitive server detail'); await tester.pumpWidget(_app(AsyncError<ChallengeDetailViewData>(error, StackTrace.empty))); await tester.pump(); expect(find.text('we couldn’t load this challenge. please try again.'), findsOneWidget); expect(find.text('sensitive server detail'), findsNothing); expect(find.text('try again'), findsOneWidget); });
  testWidgets('unavailable state renders offline presentation without rewards', (tester) async { const error = TrustedBackendException(TrustedBackendErrorCode.unavailable, 'service unavailable'); await tester.pumpWidget(_app(AsyncError<ChallengeDetailViewData>(error, StackTrace.empty))); await tester.pump(); expect(find.text("you're offline. pulse will retry when you're connected."), findsOneWidget); expect(find.textContaining('no completion or reward was recorded'), findsOneWidget); });
  test('CompleteChallenge delegates to TrustedChallengeBackend and preserves authoritative values', () async { final backend = FakeTrustedChallengeBackend(CompleteChallengeResult(activityCompleted: true, alreadyCompleted: false, xpAwarded: 75, previousXP: 90, newXP: 165, previousStreak: 3, newStreak: 4, longestStreak: 4, previousLevel: 1, newLevel: 2, leveledUp: true, newAchievements: const ['FIRST_STEP'], challengeId: 'challenge-1')); final result = await CompleteChallenge(backend: backend).call(); expect(backend.completeCalls, 1); expect(backend.lastIdempotencyKey, isNull); expect(result.xpAwarded, 75); expect(result.currentXP, 165); expect(result.currentStreak, 4); expect(result.newLevel, 2); expect(result.newAchievements, ['FIRST_STEP']); expect(result.events.whereType<Object>(), isNotEmpty); });
